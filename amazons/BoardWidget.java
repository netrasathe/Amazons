...
class BoardWidget extends Pad {
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (int j = 0; j < 10; j++) {
            if (j % 2 == 0) {
                this.fillInEvenRow(g, j);
            } else {
                this.fillInOddRow(g, j);
            }
        }
    }

    /** Fill in the dark squares for even row.
     * @param g g
     * @param y y
     * */
    private void fillInEvenRow(Graphics2D g, int y) {
        g.setColor(DARK_SQUARE_COLOR);
        int yCoord = SQUARE_SIDE * y;
        for (int i = 1; i < 10; i += 2) {
            g.fillRect(SQUARE_SIDE * i, yCoord, SQUARE_SIDE, SQUARE_SIDE);
        }
    }

    /** Fill in the dark squares for odd row.
     * * @param g g
     * * @param y y
     * */
    private void fillInOddRow(Graphics2D g, int y) {
        g.setColor(DARK_SQUARE_COLOR);
        int yCoord = SQUARE_SIDE * y;
    ...
    /** Draw the board as it is right now.
     * * @param g g
     * */
    private void drawPiecesOnBoard(Graphics2D g) {
        Square currentSquare;
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                currentSquare = Square.sq(i, j);
                Piece piece = _board.get(currentSquare);
                if (piece == WHITE || piece == BLACK) {
                    drawQueen(g, currentSquare, piece);
                } else if (piece == SPEAR) {
                    drawSpear(g, currentSquare);
                }
            }
        }
    }
    ...
    private void click(Square s) {
        if (origin == null) {
            if (_board.isLegal(s)) {
                origin = s;
                System.out.println(_board.turn().toName() + " queen on "
                        + origin.toString() + " selected");
            } else {
                System.out.println("Click on a "
                        + "square with a " + _board.turn().toName() + " queen");
            }
        } else {
            if (desttt == null) {
                if (_board.isLegal(origin, s)) {
                    desttt = s;
                    _board.put(_board.get(origin), desttt);
                    _board.put(EMPTY, origin);
                } else {
                    origin = null;
                    System.out.println(s + " is not a valid queen move."
                            + " Restart your move");
                }
            } else {
                _board.put(_board.turn(), origin);
                _board.put(EMPTY, desttt);
                if (_board.isLegal(origin, desttt, s)) {
                    _board.makeMove(origin,
                            desttt, s);
                    origin = null;
                    desttt = null;
                    System.out.println("Move completed. It is "
                            + _board.turn().toName()
                            + "'s move now");
                } else {
                    origin = null;
                    ...
                    System.out.println(s
                            + " is not a valid spear throw. Restart your move");
                }
            }
        }
        if (_board.winner() != null) {
            System.out.println(_board.winner().toName() + " wins!");
            System.exit(1);
        }
        repaint();
    }
    ...
    /** Square piece is starting at. */
    private Square origin = null;

    /** Square piece is moving to. */
    private Square desttt = null;
}
